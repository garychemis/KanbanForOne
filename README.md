...
- 字段包含项目、提出专业、接收专业、接收人、提出日期、条件名称、版次，以及可增删编辑的多组图幅和图纸数量；新增规格默认数量为 1，数量必须为大于 0 的整数，重复图幅禁止保存，自定义图幅不能包含 `|`。
...
- 支持导出“设计条件汇总”和“设计条件明细”两个 Excel 工作表，明细按一种图幅一行展开。
...
1. 打开“设计条件归档”。
2. 点击“新建设计条件”，填写流转专业、提出日期和版次，并在图幅表格中添加一种或多种图幅及对应数量。
3. 根据需要选择多个条件文件并保存。
...
设计条件使用独立 SQLite 数据库 `DesignConditions.db`，当前 schema 版本为 `3`，不属于主数据库 schema。V3 在单条条件记录中以分隔文本保存多组图幅和逐规格数量，同时保留整数总张数用于高效汇总。
